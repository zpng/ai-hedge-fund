...
def fundamentals_analyst_agent(state: AgentState):
    ...
    for ticker in tickers:
        ...
        signals.append("看涨" if profitability_score >= 2 else "看跌" if profitability_score == 0 else "中立")
        ...
        signals.append("看涨" if growth_score >= 2 else "看跌" if growth_score == 0 else "中立")
        ...
        signals.append("看涨" if health_score >= 2 else "看跌" if health_score == 0 else "中立")
        ...
        signals.append("看跌" if price_ratio_score >= 2 else "看涨" if price_ratio_score == 0 else "中立")
        ...
        reasoning_text_parts.append(f"\n信号统计: 看涨{bullish_signals}个, 看跌{bearish_signals}个, 中性{signals.count('中立')}个")
